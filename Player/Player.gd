...
var MagicMissile = preload("res://Player/MagicMissile.tscn")
...
func _process(delta):
	...
	if Input.is_action_just_released("fire"):
		...
		var dir;
		if(sprite.flip_h):
			bulletSpawn = $Sprite/BulletSpawn_L;
			dir = 180;
		else:
			bulletSpawn = $Sprite/BulletSpawn_R;
			dir = 0;
		
		mm.start(bulletSpawn.global_position, dir)
